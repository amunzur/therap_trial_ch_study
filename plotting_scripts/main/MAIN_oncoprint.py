...
path_progression_ch=f"{project_dir}/CH_progression.csv"
...
# Load CH dataset
ch=pd.read_csv(path_progression_ch)
ch=ch[ch["Independently detected at baseline"]==False]

# Load sample info and get arm-specific subsets
sample_info = pd.read_csv(path_sample_info, sep="\t")
prog_info = sample_info[sample_info["Timepoint"] == "FirstProgression"]
prog_info_lu = prog_info[prog_info["Arm"] == "LuPSMA"]
prog_info_caba = prog_info[prog_info["Arm"] == "Cabazitaxel"]
n_total_dict = {"LuPSMA": len(prog_info_lu), "Cabazitaxel": len(prog_info_caba)}

# Count mutations per patient and gene
gene_order = ["PPM1D", "TP53", "CHEK2", "ATM", "DNMT3A", "TET2", "ASXL1"]
...
def enumerate_samples(df, arm_name):
    ...
    no_ch = [pid for pid in (prog_info[prog_info["Arm"] == arm_name]["Patient_id"]) if pid not in gene_counts["Patient_id"].unique()]
...
gs_lu=gridspec.GridSpecFromSubplotSpec(3, 2, width_ratios=[1, 0.3], height_ratios=[4, 3, 0.4], wspace=0, subplot_spec=gs_outer[0])
gs_caba=gridspec.GridSpecFromSubplotSpec(3, 2, width_ratios=[1, 0.3], height_ratios=[4, 3, 0.4], wspace=0, subplot_spec=gs_outer[1])

bar_height=0.8
bar_width=0.9

gene_dict={"ddr": ["ATM", "CHEK2", "TP53", "PPM1D"], "dta": ["ASXL1", "TET2", "DNMT3A"]}

for i, arm, arm_gs in zip([0, 2], ['LuPSMA', 'Cabazitaxel'], [gs_lu, gs_caba]):
    ax_age=plt.subplot(arm_gs[2, 0])
    ...
    for j, gene_group in enumerate(["ddr", "dta"]):
        ...
        for k, gene in enumerate(gene_list):
            ...
            ax_swarm.set_xlim(np.log10(0.22), np.log10(25))
            ax_swarm.set_xticks([np.log10(0.25), np.log10(2), np.log10(10), np.log10(25)])
            ax_swarm.set_xticklabels(["0.25", "2", "10", "25"], fontsize=6)
...
# fig.suptitle("Progression mutations not detected at baseline")
fig.savefig(os.path.join(dir_figures, "MAIN_oncoprint.pdf"), transparent=True)
fig.savefig(os.path.join(dir_figures, "MAIN_oncoprint.png"))